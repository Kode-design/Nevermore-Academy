...
export default class DinerScene extends BaseGameplayScene {
    ...
    create() {
        ...
        // We need to forward our interaction prompt events to UI
        ...
        // Listen for dialogue complete from UI
        uiScene.events.on('dialogue-complete', () => {
             // Only handle if this scene is active?
             if (this.scene.isActive()) {
                 this.events.emit('dialogue-complete'); // Parent handles state
    ...
    triggerDialogue(dialogueData) {
        // Emit to UI Scene directly or via event
        this.scene.get('UIScene').events.emit('start-dialogue', dialogueData);
        // Parent class listens to 'dialogue-start' if we emit it locally,
        // OR we just manually set state. Parent listens to 'dialogue-start' on ITSELF.
